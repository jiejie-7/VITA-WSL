...
def worker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        elif cmd == 'get_malicious_mask':
            if hasattr(env, 'get_malicious_mask'):
                mask = env.get_malicious_mask()
            else:
                mask = getattr(env, '_malicious_mask', None)
            remote.send(mask)
        else:
            raise NotImplementedError
...
class ShareSubprocVecEnv(ShareVecEnv):
    ...
    def get_malicious_mask(self):
        for remote in self.remotes:
            remote.send(('get_malicious_mask', None))
        ...
        for m in masks:
            if m is None:
                arrs.append(np.zeros((n_agents,), dtype=bool))
            else:
                arrs.append(np.asarray(m, dtype=bool))
